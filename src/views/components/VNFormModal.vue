...
<template>
    ...
    content-class="relative flex flex-col 2xl:w-1/2 max-h-full mx-4 p-4 border-transparent rounded-lg dark:border-gray-800 rounded bg-white dark:bg-gray-900">
        ...
        <div class="overflow-y-auto my-5">
            <v-form ref="form" v-model="isGeneralInfoValid" class="px-2" lazy-validation>
                ...
                <v-form v-model="isVNUrlValid">
                    ...
                    <v-select clearable v-model="vn.provider" label="Provider" :items=providerLists required></v-select>
                    <v-text-field v-model="vn.url" label="URL" :rules="urlRule" required></v-text-field>
                    <button @click="isVNUrlEdit ? updateUrl(editId) : addToList()" type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-md px-5 py-2.5 mr-2 mb-2 disabled:opacity-50 disabled:pointer-events-none" :disabled="!isVNUrlValid">{{ isVNUrlEdit ? 'Edit' : 'Add' }}</button>
                </v-form>
                <!-- VN URL Table -->
                <v-table theme="light" class="mt-2">
                    ...
                    <tbody>
                        <tr v-for="(vnLink, id) in vnDataUrls" :key="id" :ref="id" class="test">
                            <td>{{ vnLink.language }}</td>
                            <td>{{ vnLink.type }}</td>
                            <td>{{ vnLink.provider }}</td>
                            <td><p class="lg:max-w-[2rem] xl:max-w-[10rem] 2xl:max-w-md truncate ...">{{ vnLink.url }}</p></td>
                            <td>
                                <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center mr-2 disabled:opacity-50 disabled:pointer-events-none" @click="editUrl(id)" :disabled="isVNUrlEdit">
                                    <v-icon>mdi-pencil-outline</v-icon>
                                </button>
                                <button class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center mr-2 disabled:opacity-50 disabled:pointer-events-none"  @click="deleteUrl(id)" :disabled="isVNUrlEdit">
        ...
        <div class="flex-shrink-0 flex justify-center items-center pt-4 space-x-4">
            <button @click="$emit('store', code, vnDataUrls, close)" type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-md px-5 py-2.5 mr-2 mb-2 disabled:opacity-50 disabled:pointer-events-none" :disabled="!isGeneralInfoValid">Save</button>
