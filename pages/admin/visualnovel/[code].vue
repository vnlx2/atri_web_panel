...
await useLazyAsyncData('loadVN', async() => {
...
function setAltImage(e: any) {
  ...
  e.target.srcset = "/assets/img/not_found_img.jpg 1x, /assets/img/not_found_img.jpg 2x"
}
</script>
...
<template>
  ...
  <h1 class="font-sans font-medium text-3xl my-5 rounded-lg">{{ visualNovelForm.title }}</h1>
  <div>
      ...
      <div class="flex">
        ...
        <div class="w-2/3 max-w-4xl px-5 space-y-5">
          ...
          <div class="grid grid-cols-3">
            ...
            <p class="text-lg">{{ (visualNovelForm.aliases === "") ? '-' : visualNovelForm.aliases }}</p>
          </div>
          <!-- Length -->
          <div class="grid grid-cols-3">
            <h6
          ...
          <div class="grid">
            ...
            <p class="text-lg text-justify whitespace-pre-line">{{ visualNovelForm.description }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md px-10 mt-10 py-5">
      ...
      <table :class="['w-full min-w-max table-auto text-left']">
        ...
        <tbody>
          <tr v-if=" visualNovelForm.downloadUrl.length > 0" v-for="(vnDl, index) in visualNovelForm.downloadUrl" :key="index">
            ...
            <td class="p-4 border-b border-blue-gray-50">
              <a :href="vnDl.url" class="text-blue-500 underline">{{ vnDl.url }}</a>
            </td>
          </tr>
          <tr v-else>
            <td colspan="4">
