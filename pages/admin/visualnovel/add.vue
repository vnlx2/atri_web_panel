...
function addUrl() {
  if (editedRow.index.value === (visualNovelForm.value.downloadUrl.length - 1) && editedRow.isAdded.value) {
...
function editUrl(index: number) {
  if (editedRow.isEdited.value && editedRow.index.value === visualNovelForm.value.downloadUrl.length - 1) {
    removeUrl(visualNovelForm.value.downloadUrl.length - 1);
  }
  vnUrlForm.language = visualNovelForm.value.downloadUrl[index]['language'] as string;
...
async function save() {
  try {
    const response = await vnController.createVisualNovel(visualNovelForm.value);
    if (!(response?.status.value === "success")) {
      const errors: IValidationError[] | any = response?.error.value?.data.errors;
