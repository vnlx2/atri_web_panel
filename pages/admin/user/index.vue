...
const { users } = storeToRefs(userController);
...
<template>
  <h1 class="font-sans font-medium text-3xl my-5 rounded-lg">User Management</h1>
      ...
      <table class="table-auto w-full">
        <thead>
          <tr class="border-y border-gray-300">
            <th class="py-2 text-left pl-5">Username</th>
            <th class="py-2 text-left pl-5">Role</th>
        ...
        <tbody>
          ...
          >
            <td class="py-2 pl-5">{{ user.username }}</td>
            <td class="py-2 pl-5">{{ user.role.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\b\w/g, (match) => match.toUpperCase()) }}</td>
            <td class="py-2 pl-5">
              <button
                @click="
                  navigateTo(`/admin/user/edit?code=${user.id}`)
                "
